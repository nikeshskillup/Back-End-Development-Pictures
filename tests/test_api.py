...
def test_post_picture(picture, client):
    ...
    res = client.post("/picture", data=json.dumps(picture), content_type="application/json")
    assert res.status_code == 201  # Change the expected status code to 201
    assert res.json['id'] == picture['id']

def test_post_picture_duplicate(picture, client):
    # create a brand new picture to upload
    res = client.post("/picture", data=json.dumps(picture), content_type="application/json")
    assert res.status_code == 302  # Change the expected status code to 302
...
def test_update_picture_by_id(client, picture):
    ...
    res = client.put(f'/picture/{id}', data=json.dumps(res_picture), content_type="application/json")
    assert res.status_code == 201  # Added an assert statement
    res = client.get(f'/picture/{id}')
